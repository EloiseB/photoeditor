...
function random (img){
    ...
    for (var i=0; i<all; i+=4){
        var originalRGBA = new RGBA(data[i], data[i+1], data[i+2], data[i+3]);
        var randomRGBA = randomPixel(originalRGBA);
        setPixel(data, i, randomRGBA)
    }
    ImageUtils.putPixels(pixels, img.width, img.height);
}

function randomPixel(colour){
    var modifiedRed = colour.red * 0.1234 + colour.green * 0.4888888888888 + colour.blue * 0.2365;
    var modifiedGreen = colour.red * 0.9 + colour.green * 0.2 + colour.blue * 0.9876543210;
    var modifiedBlue = colour.red * 0.999+ colour.green * 0.254 + colour.blue * 0.456;
    return new RGBA(modifiedRed, modifiedGreen, modifiedBlue, colour.alpha);
}
...
$(document).ready(function() {
    var img = new Image();
    img.src = "img/cat.jpg";
    var colour = new RGBA(50, 0, 150, 255);
